...
黑馬記念挑戰組使用的瑞士制配對與計分 Web App。目前已完成單場賽制與 Phase 2 的 Firebase 基礎：可建立三輪賽事、以三位或五位評審計分、抽選對戰、處理 MC 與棄賽、查看多層排名，並可選擇使用瀏覽器本機存檔或 Firestore 即時同步。
...
- Firebase 管理 token 登入、雲端賽事建立、公開狀態切換、離線快取及操作稽核。
