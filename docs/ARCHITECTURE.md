...
完賽版本在 `saveTournamentVersion` 中已經與 `resultLocked=true` 原子寫入。因此從完賽頁返回系列首頁或登出屬於純導航，不另行寫回 tournament 文件；報名中或進行中的場次仍要在離開前送完 pending 資料。
...
- 永久刪除採可重複執行的分段流程，先下架、再清除 audit／versions，最後移除賽事及系列卡片；詳細行為見 [TOURNAMENT_LIFECYCLE.md](TOURNAMENT_LIFECYCLE.md)。
